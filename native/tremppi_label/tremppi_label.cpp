	...
	for (const RegInfo & reg_info : reg_infos) {
		for (const auto & regulator : reg_info.regulators) {
			...
			for (const ActLevel & threshold : regulator.second) {
				string column_name = "L_" + reg_name + "_" + to_string(threshold) + "_" + reg_info.name;
...
char getSign(const vector<Levels> & contexts, const vector<vector<size_t> > & context_pairs, const Levels & params, const size_t reg_no, const ActLevel threshold) {
	const bool activating = isActivating(contexts, context_pairs, params, reg_no, threshold);
	const bool inhibiting = isActivating(contexts, context_pairs, params, reg_no, threshold);
			...
			for (auto sel_ID : sel_IDs) {
				...
				for (const size_t reg_no : cscope(reg_info.regulators)) {
					...
					for (const ActLevel threshold : reg_info.regulators.at(reg_ID)) {
						update += " L_" + reg_infos[reg_ID].name + "_" + to_string(threshold) + "_" + reg_info.name + " = " + getSign(contexts, context_pairs, params, reg_no, threshold) + ",";
					}
				}
